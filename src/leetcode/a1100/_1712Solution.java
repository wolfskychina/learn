...
/**
 * 将数组分为三个元素和为非递减的子数组的方案数量
 */
public class _1712Solution {
    ...
    public int waysToSplit(int[] nums) {
        ...
        for (int i = 0; i < n && sums[i] <= t; i++) {
            ...
            // 在 [i+1, n] 中二分查找 sums[i] * 2，sums[i] 为到 i 为止元素和，因为是前缀数组，因而应该查找 sum(left) +
            // sum(mid)
            int left = lowerBound(i + 1, n - 1, sums, sums[i] * 2);
            // 二分查找第二刀的最大值：sum(mid) == sum(mid + right) / 2
            // 在 [i+1, n] 中二分查找 sums[i] + (sums[n - 1] - sums[i]) / 2)，因为是前缀数组，因而应该查找
            // sum(left) + sum(mid + right) / 2
